        ...
        _buildShape: function () {
            ...
            for (var i = 0, l = series.length; i < l; i++) {
                if (series[i].type === ecConfig.CHART_TYPE_PIE) {
                    ...
                    this._selected[i] = [];
					var bDrawInner = series[i].innerRing ? 10 : 0;
					var bDrawOutter = series[i].outterRing ? 10 : 0;
					var strColor = (series[i].innerRing && series[i].innerRing) ? (series[i].calculableHolderColor || this.ecTheme.calculableHolderColor || ecConfig.calculableHolderColor) : 'rgba(0,0,0,0)';
                    if (this.deepQuery([
                            series[i],
                            this.option
                        ], 'calculable')) {
                        pieCase = {
                            ...
                            style: {
                                x: center[0],
                                y: center[1],
                                r0: radius[0] <= bDrawInner ? 0 : radius[0] - bDrawInner,
                                r: radius[1] + bDrawOutter,
                                brushType: 'stroke',
                                lineWidth: 1,                                
